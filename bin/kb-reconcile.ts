...
import { projectSourcePage, reconcilePage, slugMapsFromManifest } from '../src/reconcile/reconcile-projection';
...
function main(): number {
  ...
  if (desiredFile) {
    ...
    desiredContent = projectSourcePage(readFileSync(source, 'utf8'), maps?.publishedSlugs, maps?.titleToSlug);
  }

  const res = reconcilePage({ desiredContent, pageFile: page, sidecarFile: sidecar, dryRun: Boolean(args.plan), prune: Boolean(args.prune) });

  if (args.json) { console.log(JSON.stringify(res, null, 2)); return 0; }

  const order: Record<string, number> = { conflict: 0, drifted: 1, delete: 2, orphan: 3, create: 4, update: 5, unchanged: 6 };
  ...
  console.log(
    '\n' +
      [tally('applied', res.applied), tally('created', res.created), tally('deleted', res.deleted), tally('preserved', res.preserved), tally('skipped', res.skipped)]
        .filter(Boolean)
        .join(' · ') +
